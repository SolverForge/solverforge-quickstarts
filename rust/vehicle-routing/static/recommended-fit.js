...
function addNewVisit(id, lat, lng, map, marker) {
  ...
  let minStartPicker = flatpickr("#inputMinStartTime", {
    ...
    defaultDate: parseTimeToDateTime(defaultType.windowStart).format(
  ...
  let maxEndPicker = flatpickr("#inputMaxStartTime", {
    enableTime: true,
    dateFormat: "Y-m-d H:i",
    defaultDate: parseTimeToDateTime(defaultType.windowEnd).format(
      JSJoda.DateTimeFormatter.ofPattern("yyyy-M-d HH:mm"),
    ),
  });
  ...
  $(".customer-type-btn").click(function () {
    ...
    // Update time windows
    minStartPicker.setDate(
      parseTimeToDateTime(config.windowStart).format(
        JSJoda.DateTimeFormatter.ofPattern("yyyy-M-d HH:mm"),
      ),
    );
    maxEndPicker.setDate(
      parseTimeToDateTime(config.windowEnd).format(
        JSJoda.DateTimeFormatter.ofPattern("yyyy-M-d HH:mm"),
      ),
    );
    ...
    $("#durationHint").text(
      `(${config.minService}-${config.maxService} min typical)`,
    );
  ...
  visitModalFooter.append(
    "<button id='recommendationButton' type='button' class='btn btn-success'><i class='fas fa-arrow-right'></i> Get Recommendations</button>",
  );
...
function requestRecommendations(visitId, solution, endpointPath) {
  $.post(
    endpointPath,
    JSON.stringify({ solution: stripGeometries(solution), visitId }),
    function (recommendations) {
      const visitModalContent = $("#newVisitModalContent");
      visitModalContent.children().remove();

      if (!recommendations || recommendations.length === 0) {
        visitModalContent.append(
          "<div class='alert alert-warning'>No recommendations available. The recommendation API may not be fully implemented.</div>",
        );
        const visitModalFooter = $("#newVisitModalFooter");
        visitModalFooter.children().remove();
        visitModalFooter.append(
      ...
      visitModalFooter.children().remove();
      visitModalFooter.append(
        "<button id='applyRecommendationButton' type='button' class='btn btn-success'><i class='fas fa-check'></i> Accept</button>",
      );
      $("#applyRecommendationButton").click((_) =>
        applyRecommendationModal(recommendations),
      );
    },
...
function applyRecommendation(
  ...
  $.post(
    endpointPath,
    JSON.stringify({
      solution: stripGeometries(solution),
      visitId,
      vehicleId,
      index,
    }),
