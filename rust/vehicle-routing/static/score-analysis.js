function analyzeScore(solution, endpointPath) {
  ...
  if (solution.score == null) {
    scoreAnalysisModalContent.text(
      "Score not ready for analysis, try to run the solver first or wait until it advances.",
    );
  } else {
...
function visualizeScoreAnalysis(
  ...
  $.put(
    endpointPath,
    JSON.stringify(stripGeometries(solution)),
    function (scoreAnalysis) {
      ...
      const analysisTable = $(`<table class="table"/>`).css({
        textAlign: "center",
      });
      const analysisTHead = $(`<thead/>`).append(
        $(`<tr/>`)
          .append($(`<th></th>`))
...
function compareConstraintsBySeverity(a, b) {
  let aComponents = getScoreComponents(a.score),
    bComponents = getScoreComponents(b.score);
...
function getScoreComponents(score) {
  ...
  $.each(
    [...score.matchAll(/(-?[0-9]+)(hard|medium|soft)/g)],
    function (i, parts) {
...
function visualizeConstraintAnalysis(
  analysisTBody,
  constraintIndex,
  constraintAnalysis,
  ...
  let icon =
    constraintAnalysis.type == "hard" && constraintAnalysis.implicitScore < 0
      ? '<span class="fas fa-exclamation-triangle" style="color: red"></span>'
      : "";
  if (!icon)
    icon =
      constraintAnalysis.weight < 0 && constraintAnalysis.matches.length == 0
    ...
    .append(
      $(`<td/>`).text(
        recommendation
          ...
          : constraintAnalysis.implicitScore,
      ),
    );
