...
//! # Design
//!
//! Uses SolverForge's fluent constraint API with list variables.
//! Vehicles have a list of visit indices as their planning variable.
...
/// ```
/// Location is a problem fact but we need custom PartialEq/Eq based on index only
/// (f64 doesn't implement Eq, and we want identity based on index).
...
pub struct Vehicle {
    ...
    /// Ordered list of visit indices (the route) - the planning list variable.
    #[serde(default)]
...
/// ```
/// VRP uses list variables (visits assigned to vehicles).
/// The solve() method is generated by the macro using k-opt from the public API.
#[planning_solution(serde, constraints = "crate::constraints::define_constraints")]
...
pub struct VehicleRoutePlan {
    ...
    /// All customer visits (element type for list variable).
    #[problem_fact_collection]
    pub visits: Vec<Visit>,
    /// All vehicles (planning entities with list variable).
