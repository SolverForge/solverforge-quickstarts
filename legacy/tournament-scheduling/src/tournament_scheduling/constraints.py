...
def one_assignment_per_date_per_team(
    constraint_factory: ConstraintFactory,
) -> Constraint:
    return (
        constraint_factory.for_each(TeamAssignment)
        .join(
            TeamAssignment,
...
def unavailability_penalty(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        constraint_factory.for_each(UnavailabilityPenalty)
        .if_exists(
            TeamAssignment,
            Joiners.equal(
                lambda unavailability_penalty: unavailability_penalty.team,
                lambda team_assignment: team_assignment.team,
            ),
            Joiners.equal(
                lambda unavailability_penalty: unavailability_penalty.day,
...
def fair_assignment_count_per_team(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        ...
        .penalize_decimal(
            HardMediumSoftDecimalScore.ONE_MEDIUM, lambda balance: balance.unfairness()
        )
        .justify_with(
            lambda balance, score: LoadBalanceJustification(balance.unfairness())
        )
...
def evenly_confrontation_count(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        ...
        .group_by(
            ConstraintCollectors.load_balance(
                lambda assignment, other_assignment: (
                    assignment.team,
                    other_assignment.team,
                )
            )
        )
        .penalize_decimal(
            HardMediumSoftDecimalScore.ONE_SOFT, lambda balance: balance.unfairness()
        )
        .justify_with(
            lambda balance, score: LoadBalanceJustification(balance.unfairness())
        )
