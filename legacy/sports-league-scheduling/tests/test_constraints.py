...
constraint_verifier = ConstraintVerifier.build(
    define_constraints, LeagueSchedule, Match
)
...
def test_matches_same_day():
    ...
    match1 = Match(
        id="1", home_team=home_team, away_team=rival_team, round=Round(index=0)
    )
    match2 = Match(
        id="2", home_team=home_team, away_team=rival_team, round=Round(index=0)
    )
    match3 = Match(id="3", home_team=home_team, away_team=rival_team)

    constraint_verifier.verify_that(matches_on_same_day).given(
    ...
    match2 = Match(
        id="2", home_team=rival_team, away_team=other_team, round=Round(index=0)
    )
...
def test_multiple_consecutive_home_matches():
    ...
    constraint_verifier.verify_that(multiple_consecutive_home_matches).given(
        *matches, home_team, rival_team
    ).penalizes_by(4)
...
def test_multiple_consecutive_away_matches():
    ...
    constraint_verifier.verify_that(multiple_consecutive_away_matches).given(
        *matches, home_team, rival_team
    ).penalizes_by(4)
...
def test_repeat_match_on_next_day():
    ...
    constraint_verifier.verify_that(repeat_match_on_the_next_day).given(
        *matches
    ).penalizes_by(1)
...
def test_away_to_away_hop():
    ...
    matches = [
        Match(
            ...
            away_team=Team(id="1", name="TeamA"),
            round=Round(index=0),
        ),
        Match(
            id="2",
...
def test_away_to_home_hop():
    ...
    matches = [
        ...
        Match(
            id="2",
            home_team=home_team,
            away_team=Team(id="3", name="TeamC"),
            round=Round(index=1),
        ),
    ]
    # match with the home team
...
def test_away_to_end_hop():
    ...
    matches = [
        Match(
            id="1",
            home_team=home_team,
            away_team=Team(id="2", name="TeamB"),
            round=Round(index=0),
        ),
        Match(id="2", home_team=third_team, away_team=home_team, round=Round(index=1)),
    ]
