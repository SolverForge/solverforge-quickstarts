...
def are_teams_overlapping(match1: Match, match2: Match) -> bool:
    return (
        ...
        or match1.home_team == match2.away_team
        or match1.away_team == match2.home_team
        or match1.away_team == match2.away_team
    )


def multiple_consecutive_home_matches(
    constraint_factory: ConstraintFactory,
) -> Constraint:
    return (
        constraint_factory.for_each(Match)
        .join(Team, Joiners.equal(lambda match: match.home_team, lambda team: team))
        .group_by(
            lambda match, team: team,
            ConstraintCollectors.to_consecutive_sequences(
                lambda match, team: match.round, lambda match_round: match_round.index
            ),
        )
...
def multiple_consecutive_away_matches(
    constraint_factory: ConstraintFactory,
) -> Constraint:
    return (
        constraint_factory.for_each(Match)
        .join(Team, Joiners.equal(lambda match: match.away_team, lambda team: team))
        .group_by(
            lambda match, team: team,
            ConstraintCollectors.to_consecutive_sequences(
                lambda match, team: match.round, lambda match_round: match_round.index
...
def repeat_match_on_the_next_day(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        ...
        .if_exists(
            ...
            Joiners.equal(
                lambda match: match.round.index + 1, lambda match: match.round.index
            ),
        )
        .penalize(HardSoftScore.ONE_HARD)
...
def start_to_away_hop(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        ...
        .if_not_exists(
            Round,
            Joiners.equal(
                lambda match: match.round.index - 1,
                lambda match_round: match_round.index,
            ),
        )
        .penalize(
            HardSoftScore.ONE_SOFT,
            lambda match: match.away_team.get_distance(match.home_team),
        )
...
def home_to_away_hop(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        ...
        .penalize(
            HardSoftScore.ONE_SOFT,
            lambda match, other_match: match.home_team.get_distance(
...
def away_to_home_hop(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        ...
        .penalize(
            HardSoftScore.ONE_SOFT,
            lambda match, other_match: match.home_team.get_distance(match.away_team),
        )
...
def away_to_end_hop(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        ...
        .penalize(
            HardSoftScore.ONE_SOFT,
            lambda match: match.home_team.get_distance(match.away_team),
        )
...
def classic_matches(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        constraint_factory.for_each(Match)
        .filter(
            lambda match: match.classic_match and not match.round.weekend_or_holiday
        )
