...
def required_skill(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        constraint_factory.for_each(FlightAssignment)
...
def flight_conflict(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        constraint_factory.for_each_unique_pair(
            ...
            Joiners.overlapping(
                lambda fa: fa.flight.departure_utc_date_time,
...
def transfer_between_two_flights(constraint_factory: ConstraintFactory) -> Constraint:
    return (
        ...
        .if_not_exists(
            ...
            Joiners.filtering(
                lambda fa1, fa2, other_fa: other_fa.id != fa1.id
                and other_fa.id != fa2.id
                and other_fa.get_departure_utc_date_time()
                >= fa1.get_departure_utc_date_time()
                and other_fa.get_departure_utc_date_time()
                < fa2.get_departure_utc_date_time()
            ),
        )
        .filter(
            lambda fa1, fa2: fa1.flight.arrival_airport != fa2.flight.departure_airport
        )
...
def first_assignment_not_departing_from_home(
    ...
    return (
        ...
        .if_not_exists(
            ...
            Joiners.greater_than(
                lambda emp, fa: fa.get_departure_utc_date_time(),
...
def last_assignment_not_arriving_at_home(
    constraint_factory: ConstraintFactory,
) -> Constraint:
    return (
        constraint_factory.for_each(Employee)
